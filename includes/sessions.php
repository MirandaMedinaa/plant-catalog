...
function login_url()
{
  ...
  // Add a login query string parameter
  $params = $_GET;
  $params['login'] = '';

  // Add logout param to current page URL.
  $login_url = htmlspecialchars($request_uri) . '?' . http_build_query($params);
...
?>
  ...
  <form class="login" action="<?php echo htmlspecialchars($action) ?>" method="post" novalidate>
    ...
    <div class="align-right">
      <button name="login" type="submit" style="margin-left:80px; margin-bottom: 15px;">Sign In</button>
    </div>
  </form>
<?php
}
